 ...
 */
    ...
    if ((args->iop->flags & LIBIO_FLAGS_READ) && cptr->pDeviceFlow && cptr->pDeviceFlow->deviceStartRemoteTx) {
    ...
    void* arg)
{
     ...
     */
    if ((current_tty->refcount == 1)) {
        if ((args->iop->flags & LIBIO_FLAGS_READ) && cptr->pDeviceFlow && cptr->pDeviceFlow->deviceStopRemoteTx) {
    ...
    void* arg)
{
    ...
    for (minor = 0; minor < Console_Port_Count; minor++) {
        ...
        if ((!port->deviceProbe || port->deviceProbe(minor)) && port->pDeviceFns->deviceProbe(minor)) {
