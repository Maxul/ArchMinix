 ...
 */
#if defined(BSP_FEATURE_IRQ_EXTENSION) || (CPU_SIMPLE_VECTORED_INTERRUPTS != TRUE)
    ...
    rtems_vector_number vector)
{
...
#if CLOCK_DRIVER_USE_FAST_IDLE
    {
        ...
        uint32_t interval = (uint32_t)((tc->tc_frequency * us_per_tick) / 1000000);
...
static void
c6678_clock_install_isr(
    rtems_isr_entry New_ticker,
    rtems_isr_entry Old_ticker)
{
     ...
     *  wants a clock tick.
     */
    Old_ticker = set_vector(New_ticker, CLOCK_VECTOR, 1);
    rtems_interrupt_catch(Clock_isr, 14, &Old_ticker);
}
    ...
    void* pargp)
{
     ...
     */
    (void)Old_ticker;
    c6678_clock_install_isr(Clock_isr, Old_ticker);
