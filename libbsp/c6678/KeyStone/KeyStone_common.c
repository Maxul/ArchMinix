    ...
    unsigned int multiplier, unsigned int divisor)
{
    ...
    gpBootCfgRegs->CORE_PLL_CTL0 = ((multiplier - 1) << PLLCTL0_BWADJ_SHIFT) | (((multiplier * 2 - 1) & 0x1FC0) << PLLCTL0_PLLM_SHIFT) | (divisor - 1);
    ...
    unsigned int multiplier, unsigned int divisor)
{
    ...
    if (0 == KeyStone_PLL_init((PLL_ControlRegs*)&gpBootCfgRegs->PA_PLL_CTL0, divisor, multiplier * 2, 2)) {
    ...
    unsigned int multiplier, unsigned int divisor)
{
    ...
    if (0 == KeyStone_PLL_init((PLL_ControlRegs*)&gpBootCfgRegs->DDR3_PLL_CTL0, divisor, multiplier, 2)) {
...
void Timer64_Init(Timer64_Config* tmrCfg)
{
    ...
    gpTimerRegs[tmrCfg->timer_num]->EMUMGT_CLKSPD = (gpTimerRegs[tmrCfg->timer_num]->EMUMGT_CLKSPD & ~(CSL_TMR_EMUMGT_CLKSPD_FREE_MASK | CSL_TMR_EMUMGT_CLKSPD_SOFT_MASK));
    ...
    if (TIMER_WATCH_DOG == tmrCfg->timerMode) {
        ...
        gpTimerRegs[tmrCfg->timer_num]->TCR = (CSL_TMR_CLOCK_INP_NOGATE << CSL_TMR_TCR_TIEN_LO_SHIFT)
...
void Service_Watchdog(int timer_num)
{
    ...
    gpTimerRegs[timer_num]->WDTCR = (CSL_TMR_WDTCR_WDKEY_CMD1 << CSL_TMR_WDTCR_WDKEY_SHIFT);
    gpTimerRegs[timer_num]->WDTCR = (CSL_TMR_WDTCR_WDKEY_CMD2 << CSL_TMR_WDTCR_WDKEY_SHIFT);
}
...
Int32 KeyStone_enable_PSC_module(Uint32 pwrDmnNum, Uint32 moduleNum)
{
    ...
    /* Return PSC status */
    if ((CSL_PSC_getPowerDomainState(pwrDmnNum) == PSC_PDSTATE_ON) && (CSL_PSC_getModuleState(moduleNum) == PSC_MODSTATE_ENABLE)) {
        /*Ready for use */
        return 0;
    } else {
        /*Return error */
...
{
    gpEDMA_CC_regs[uiCC]->TPCC_DMAQNUM[uiChannel / 8] = (gpEDMA_CC_regs[uiCC]->TPCC_DMAQNUM[uiChannel / 8] & (~(0xF << ((uiChannel & 7) * 4))))
        | (uiTC << ((uiChannel & 7) * 4));
}
/*****************************************************************************
...
{
    gpEDMA_CC_regs[uiCC]->TPCC_QUEPRI = (gpEDMA_CC_regs[uiCC]->TPCC_QUEPRI & (~(0xF << ((uiTC & 3) * 4))))
        | (uiPri << ((uiTC & 3) * 4));
}
/*****************************************************************************
 Prototype    : EDMA_init
    ...
    unsigned int byteCount, EDMA_CC_Channel_Num CC_channel, DMA_Wait wait)
{
    ...
    EDMACCRegs->PARAMSET[uiChannel].OPT = CSL_EDMA3_OPT_MAKE(CSL_EDMA3_ITCCH_DIS,
        CSL_EDMA3_TCCH_DIS,
    ...
    unsigned int MPFSR)
{
    ...
    if (MPFSR & CSL_CGEM_L2MPFSR_LOCAL_MASK) {
        ...
        master_id = (MPFSR & CSL_CGEM_L2MPFSR_FID_MASK) >> CSL_CGEM_L2MPFSR_FID_SHIFT;
...
void KeyStone_MSMC_protection_exception_handler()
{
    ...
    master_id = (gpMSMC_regs->SMPFR & CSL_MSMC_SMPFR_FMSTID_MASK) >> CSL_MSMC_SMPFR_FMSTID_SHIFT;
    priv_id = (gpMSMC_regs->SMPFR & CSL_MSMC_SMPFR_FPID_MASK) >> CSL_MSMC_SMPFR_FPID_SHIFT;
    ...
    MPU_Range_Config MPU_cfg[], Uint32 numRangeCfg)
{
    ...
    mpuRegs->INT_ENABLED_STATUS_CLEAR = CSL_MPU_INT_ENABLED_STATUS_CLEAR_ENABLED_ADDR_ERR_MASK
...
{
    if ((gpMPU_regs[uiMPU_num]->REVISION & 0xFFFF0000) != (CSL_MPU_REVISION_RESETVAL & 0xFFFF0000)) {
    ...
    gpMPU_regs[uiMPU_num]->INT_ENABLED_STATUS_CLEAR = CSL_MPU_INT_ENABLED_STATUS_CLEAR_ENABLED_ADDR_ERR_MASK
        | CSL_MPU_INT_ENABLED_STATUS_CLEAR_ENABLED_PROT_ERR_MASK;
...
void KeyStone_MPU_interrupts_enable()
{
    ...
    if ((C6670_EVM == DSP_Board_Type)
        ...
        for (i = 4; i < 6; i++) {
            ...
            if ((5 == i) && (TCI6614_EVM == DSP_Board_Type) && ((CSL_PSC_getPowerDomainState(5) != PSC_PDSTATE_ON) || (CSL_PSC_getModuleState(12) != PSC_MODSTATE_ENABLE))) {
...
void LL2_EDC_handler(Bool bCorrected)
{
    ...
    if (derr | perr | dmaerr | verr) {
        ...
        exception_record.status.ext_sts.info.memory.fault_status.L2EDSTAT = gpCGEM_regs->L2EDSTAT;

        if (bCorrected) {
            printf("  LL2 EDC (correctable) at bit %d of ", (gpCGEM_regs->L2EDSTAT & CSL_CGEM_L2EDSTAT_BITPOS_MASK) >> CSL_CGEM_L2EDSTAT_BITPOS_SHIFT);
...
void L1P_ED_handler()
{
    ...
    if (perr | dmaerr) {
        ...
        exception_record.status.ext_sts.info.memory.fault_status.L1PEDSTAT = gpCGEM_regs->L1PEDSTAT;
...
void KeyStone_SL2_EDC_handler()
{
    ...
    exception_record.status.ext_sts.info.memory.fault_status.SMESTAT = gpMSMC_regs->SMESTAT;
    ...
    */
    ...
    if (C6670_EVM == DSP_Board_Type
        ...
        */
        ...
        gCIC_EXC_EN_MASK[2] |= (1 << (90 - 32 * 2))
            | (1 << (92 - 32 * 2))
            | (1 << (94 - 32 * 2));
        gCIC_EXC_EN_MASK[3] |= (1 << (96 - 32 * 3));
    ...
    if (TCI6614_EVM == DSP_Board_Type) {
        ...
        */
        ...
        gCIC_EXC_EN_MASK[0] |= (1 << 23);
        gCIC_EXC_EN_MASK[1] |= (1 << (37 - 32));
    }
...
void KeyStone_CIC_EXC_handler()
{
    ...
    */
    ...
    if (TCI6614_EVM == DSP_Board_Type) {
        ...
        if ((status[0] & (1 << 23)) || (status[1] & (1 << (37 - 32)))) {
            for (i = 0; i < 8; i++) {
                /*MPU5 (for BCP) of TCI6614 is only accessible
                when BCP is enabled through PSC*/
                if ((5 == i) && ((CSL_PSC_getPowerDomainState(5) != PSC_PDSTATE_ON) || (CSL_PSC_getModuleState(12) != PSC_MODSTATE_ENABLE)))
                    continue;
    ...
    if (status[3] & (1 << (110 - 96))) {
        ...
        exception_record.status.ext_sts.info.memory.fault_status.DDR_IRQSTATUS_SYS = gpDDR_regs->IRQSTATUS_SYS;
...
*****************************************************************************/
void Exception_service_routine(int vector, CPU_Interrupt_frame* pContext)
{
    ...
    if (efr) {
        ...
        if (efr & CSL_CHIP_EFR_OXF_MASK) {
            ...
            if (swexc_handler && !(ntsr & 0x00010000)) {
                (*swexc_handler)(pContext);
                return;
            } else {
                EXC_swgen();
            }
        ...
        printf("Enter Exception service routine, but excpetion flage is 0!\n");
    }
    ...
    printf(" B3=0x%x, A4=0x%x, B4= 0x%x, B14= 0x%x, B15= 0x%x\n",
        pContext->REG_B3, pContext->REG_A4, pContext->REG_B4, pContext->REG_B14, pContext->REG_B15);
    ...
    if ((ntsr & CSL_CHIP_TSR_SPLX_MASK)
        ...
        || (0x800000 > NRP)) {
        printf("Exception happened at a place can not safely return!\n");
        while (1)
            ; // trap
    }
}
...
*****************************************************************************/
void Nested_Exception_service_routine(CPU_Interrupt_frame* pContext)
{
...
void KeyStone_get_device_speed_grade()
{
    ...
    if (uiSpeedCode & DEVSPEED_800_MHZ_MASK_H)
        uiDevSpeed = 800;
    else if (uiSpeedCode & DEVSPEED_1000_MHZ_MASK_H)
        uiDevSpeed = 1000;
    else if (uiSpeedCode & DEVSPEED_1200_MHZ_MASK_H)
        uiDevSpeed = 1200;
    else if (uiSpeedCode & DEVSPEED_1250_MHZ_MASK_H)
        uiDevSpeed = 1250;
    else if (uiSpeedCode & DEVSPEED_1400_MHZ_MASK_H)
        uiDevSpeed = 1400;
    else if (uiSpeedCode & DEVSPEED_1400_MHZ_MASK_L)
        uiDevSpeed = 1400;
    else if (uiSpeedCode & DEVSPEED_1250_MHZ_MASK_L)
        uiDevSpeed = 1250;
    else if (uiSpeedCode & DEVSPEED_1200_MHZ_MASK_L)
        uiDevSpeed = 1200;
    else if (uiSpeedCode & DEVSPEED_1000_MHZ_MASK_L)
        uiDevSpeed = 1000;
    else
        uiDevSpeed = 800;
