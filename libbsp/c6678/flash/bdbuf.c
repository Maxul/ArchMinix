 ...
 */
void rtems_bdbuf_show_users(const char* where, rtems_bdbuf_buffer* bd)
{
    const char* states[] = { "FR", "EM", "CH", "AC", "AM", "AE", "AP", "MD", "SY", "TR", "TP" };
...
rtems_bdbuf_media_block(const rtems_disk_device* dd, rtems_blkdev_bnum block)
{
    if (dd->block_to_media_block_shift >= 0)
         ...
         */
        return (rtems_blkdev_bnum)((((uint64_t)block) * dd->block_size) / dd->media_block_size);
}
...
rtems_bdbuf_do_init(void)
{
    ...
    bdbuf_cache.group_count = bdbuf_cache.buffer_min_count / bdbuf_cache.max_bds_per_group;
...
#define bdbuf_alloc(size) malloc(size)
...
rtems_bdbuf_swapout_write(rtems_bdbuf_swapout_transfer* transfer)
{
    ...
    if (!rtems_chain_is_empty(&transfer->bds)) {
        ...
        uint32_t media_blocks_per_block = dd->media_blocks_per_block;
        bool need_continuous_blocks = (dd->phys_dev->capabilities & RTEMS_BLKDEV_CAP_MULTISECTOR_CONT) != 0;
        ...
        while ((node = rtems_chain_get_unprotected(&transfer->bds)) != NULL) {
            ...
            if (need_continuous_blocks && transfer->write_req.bufnum && bd->block != last_block + media_blocks_per_block) {
            ...
            if (rtems_chain_is_empty(&transfer->bds) || (transfer->write_req.bufnum >= bdbuf_config.max_write_blocks))
                write = true;
         ...
         */
        if (transfer->syncing && (dd->phys_dev->capabilities & RTEMS_BLKDEV_CAP_SYNC)) {
...
rtems_bdbuf_read_ahead_task(rtems_task_argument arg)
{
    ...
    while (bdbuf_cache.read_ahead_enabled) {
        ...
        while ((node = rtems_chain_get_unprotected(chain)) != NULL) {
            rtems_disk_device* dd = RTEMS_CONTAINER_OF(node, rtems_disk_device, read_ahead.node);
            rtems_blkdev_bnum block = dd->read_ahead.next;
            rtems_blkdev_bnum media_block = 0;
            rtems_status_code sc = rtems_bdbuf_get_media_block(dd, block, &media_block);

            rtems_chain_set_off_chain(&dd->read_ahead.node);

            if (sc == RTEMS_SUCCESSFUL) {
                rtems_bdbuf_buffer* bd = rtems_bdbuf_get_buffer_for_read_ahead(dd, media_block);
