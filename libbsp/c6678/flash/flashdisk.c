...
{
    return ((pd->crc == 0xffff) && (pd->flags == 0xffff) && (pd->block == 0xffffffff)) ? true : false;
}
...
rtems_fdisk_seg_most_available(const rtems_fdisk_segment_ctl_queue* queue)
{
    ...
    while (sc) {
        if (rtems_fdisk_seg_pages_available(sc) > rtems_fdisk_seg_pages_available(biggest))
            biggest = sc;
        sc = sc->next;
    }
    ...
    uint32_t page_size)
{
    ...
    for (s = 0; s < dd->segment_count; s++) {
        ...
        count += (rtems_fdisk_pages_in_segment(sd, page_size) - rtems_fdisk_page_desc_pages(sd, page_size)) * sd->count;
    }
    return count;
}
...
rtems_fdisk_seg_write_page_desc_flags(const rtems_flashdisk* fd,
    ...
    const rtems_fdisk_page_desc* page_desc)
{
    uint32_t offset = ((page * sizeof(rtems_fdisk_page_desc)) + ((uint8_t*)&page_desc->flags) - ((uint8_t*)page_desc));
    if ((fd->flags & RTEMS_FDISK_BLANK_CHECK_BEFORE_WRITE)) {
...
rtems_fdisk_queue_segment(rtems_flashdisk* fd, rtems_fdisk_segment_ctl* sc)
{
    ...
    if (rtems_fdisk_seg_pages_available(sc) == 0) {
        ...
        while (seg) {
            if (rtems_fdisk_seg_pages_available(sc) < rtems_fdisk_seg_pages_available(seg))
                break;
            seg = seg->next;
        }
    ...
    uint32_t* pages)
{
    ...
    for (spage = 0; spage < ssc->pages; spage++) {
        ...
        if (rtems_fdisk_page_desc_flags_set(spd, RTEMS_FDISK_PAGE_ACTIVE) && !rtems_fdisk_page_desc_flags_set(spd, RTEMS_FDISK_PAGE_USED)) {
            uint32_t dst_pages;
...
rtems_fdisk_compact(rtems_flashdisk* fd)
{
    ...
    while (fd->used.head) {
        ...
        while (ssc && ((pages + ssc->pages_active) < dst_pages) && ((compacted_segs + segments) < fd->compact_segs)) {
...
rtems_fdisk_recover_block_mappings(rtems_flashdisk* fd)
{
    ...
    for (device = 0; device < fd->device_count; device++) {
        ...
        for (segment = 0; segment < fd->devices[device].segment_count; segment++) {
            ...
            sc->pages = rtems_fdisk_pages_in_segment(sd, fd->block_size) - sc->pages_desc;
    ...
    const uint8_t* buffer)
{
     ...
     */
    if (rtems_fdisk_segment_count_queue(&fd->available) <= fd->avail_compact_segs)
        rtems_fdisk_compact(fd);
        ...
        for (seg = 0; seg < fd->devices[device].segment_count; seg++) {
            ...
            for (page = 0; page < sc->pages; page++) {
                ...
                for (block = 0; block < fd->block_count; block++) {
                    if ((fd->blocks[block].segment == sc) && (fd->blocks[block].page == page) && !is_active)
                        rtems_fdisk_printf(fd,
...
rtems_fdisk_ioctl(rtems_disk_device* dd, uint32_t req, void* argp)
{
    ...
    if (sc != RTEMS_SUCCESSFUL)
        ...
        case RTEMS_BLKIO_REQUEST:
            if ((minor >= rtems_flashdisk_count) || (rtems_flashdisks[minor].device_count == 0)) {
                errno = ENODEV;
            } else {
                switch (r->req) {
    ...
    void* arg __attribute__((unused)))
{
    ...
    for (minor = 0; minor < rtems_flashdisk_configuration_size; minor++, c++) {
        ...
        sc = rtems_semaphore_create(rtems_build_name('F', 'D', 'S', 'K'), 1,
            RTEMS_PRIORITY | RTEMS_BINARY_SEMAPHORE | RTEMS_INHERIT_PRIORITY, 0, &fd->lock);
        if (sc != RTEMS_SUCCESSFUL) {
