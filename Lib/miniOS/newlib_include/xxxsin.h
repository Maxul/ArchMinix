...
static const _DATA_ACCESS FTYPE c[] = {	/* 6th-order, |x| < pi/4 (102.5%) */
#else
static const FTYPE c[] = {	/* 6th-order, |x| < pi/4 (102.5%) */
  ...
  #define CPOLY(x)	(((((c[0] * x + c[1]) * x + c[2]) * x + c[3]) * x \
		+ c[4]) * x + c[5])
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE c[] = {	/* 12th-order, |x| < pi/4 (101.0%) */
#else
...
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE c[] = {	/* 16th-order, |x| < pi/4 (100.5%) */
#else
static const FTYPE c[] = {	/* 16th-order, |x| < pi/4 (100.5%) */
  ...
  #define CPOLY(x)	FNAME(Poly)(x, c, sizeof (c) / sizeof (c[0]) - 1)
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE c[] = {	/* 24th-order, |x| < pi/4 (101.5%) */
#else
...
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE s[] = {	/* 7th-order, |x| < pi/4 (101.0%) */
#else
static const FTYPE s[] = {	/* 7th-order, |x| < pi/4 (101.0%) */
  ...
  #define SPOLY(x)	(((((s[0] * x + s[1]) * x + s[2]) * x + s[3]) * x \
		+ s[4]) * x + s[5])
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE s[] = {	/* 13th-order, |x| < pi/4 (100.1%) */
#else
static const FTYPE s[] = {	/* 13th-order, |x| < pi/4 (100.1%) */
  ...
  #define SPOLY(x)	((((((s[0] * x + s[1]) * x + s[2]) * x + s[3]) * x \
		+ s[4]) * x + s[5]) * x + s[6])
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE s[] = {	/* 15th-order, |x| < pi/4 (100.1%) */
#else
...
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE s[] = {	/* 23rd-order, |x| < pi/4 (100.0%) */
#else
static const FTYPE s[] = {	/* 23rd-order, |x| < pi/4 (100.0%) */
 ...
 #endif /* FBITS < 24 */
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE c1 = (FTYPE)(13176794.0 / 8388608.0);
static const _DATA_ACCESS FTYPE c2 = FLIT(7.5497899548918821691639751442098584e-8);

static const _DATA_ACCESS FTYPE twobypi = FLIT(0.63661977236758134307553505349005744);
static const _DATA_ACCESS FTYPE twopi = FLIT(6.2831853071795864769252867665590057);
#else
