...
void DLSYM_copy_globals(DLIMP_Dynamic_Module *dyn_module) {
	...
	if (module->gsymtab)
		memcpy(module->gsymtab, &dyn_module->symtab[global_index],
	...
	if (module->gstrtab)
		memcpy(module->gstrtab, dyn_module->strtab + dyn_module->gstrtab_offset,
				module->gstrsz);
...
/*****************************************************************************/
static BOOL breadth_first_lookup(const char* sym_name, int handle,
	...
	while (file_handle_queue.size) {
		...
		/*---------------------------------------------------------------------*/
		while (mod_node->value->file_handle != handle)
			mod_node++;
		...
		/*---------------------------------------------------------------------*/
		if (DLSYM_lookup_global_symtab(sym_name, mod_node->value->gsymtab,
				mod_node->value->gsymnum, sym_value))
			return TRUE;
...
BOOL DLSYM_global_lookup(const char *sym_name,
		DLIMP_Loaded_Module *loaded_module, Elf32_Addr *sym_value) {
	int i = 0;
	...
	if (loaded_module->direct_dependent_only) {
		...
		for (i = 0; i < loaded_module->dependencies.size; i++) {
			...
			/*------------------------------------------------------------------*/
			if (DLSYM_lookup_global_symtab(sym_name, node->value->gsymtab,
					node->value->gsymnum, sym_value))
				return TRUE;
		}
	}
...
static BOOL DLSYM_lookup_symtab(const char *sym_name, struct Elf32_Sym *symtab,
		Elf32_Word symnum, Elf32_Addr *sym_value, BOOL require_local_binding) {
	Elf32_Addr sym_idx;
	for (sym_idx = 0; sym_idx < symnum; sym_idx++) {
		...
		if ((symtab[sym_idx].st_shndx != SHN_UNDEF)
				&& ((require_local_binding
						...
						|| (!require_local_binding
								&& (ELF32_ST_BIND(symtab[sym_idx].st_info)
										!= STB_LOCAL)))
				&& !strcmp(sym_name, (char*) (symtab[sym_idx].st_name))) {
			if (sym_value)
				*sym_value = symtab[sym_idx].st_value;
			return TRUE;
...
/*****************************************************************************/
BOOL DLSYM_canonical_lookup(int sym_index, DLIMP_Dynamic_Module *dyn_module,
		Elf32_Addr *sym_value) {
	/*------------------------------------------------------------------------*/
	...
	BOOL is_def = (sym->st_shndx != SHN_UNDEF
			&& (sym->st_shndx < SHN_LORESERVE || sym->st_shndx == SHN_XINDEX));
	...
	/*------------------------------------------------------------------------*/
		...
		return DLSYM_global_lookup(sym_name, dyn_module->loaded_module,
				sym_value);
	}
}
