...
static void initialize_loaded_module(DLIMP_Dynamic_Module *dyn_module) {
	...
	DLIMP_Loaded_Module *loaded_module = dyn_module->loaded_module =
			DLIF_malloc(sizeof(DLIMP_Loaded_Module));
...
static BOOL load_static_segment(LOADER_FILE_DESC *fd,
	...
	for (i = 0; i < dyn_module->loaded_module->loaded_segments.size; i++) {
		...
		/*---------------------------------------------------------------------*/
		if (seg[i].phdr.p_flags & PF_X)
			targ_req.flags |= DLOAD_SF_executable;
...
BOOL DLIMP_update_dyntag_section_address(DLIMP_Dynamic_Module *dyn_module,
	...
	for (j = 0; j < dyn_module->loaded_module->loaded_segments.size; j++) {
		if ((dyn_module->dyntab[i].d_un.d_ptr >= seg[j].input_vaddr)
			...
			dyn_module->dyntab[i].d_un.d_ptr += (seg[j].phdr.p_vaddr
					- seg[j].input_vaddr);
			return TRUE;
		}
	}
...
/*                                                                           */
/*****************************************************************************/
static BOOL allocate_dynamic_segments_and_relocate_symbols(LOADER_FILE_DESC *fd,
		DLIMP_Dynamic_Module *dyn_module) {
	int i, j;
	DLIMP_Loaded_Segment* seg =
			(DLIMP_Loaded_Segment*) (dyn_module->loaded_module->loaded_segments.buf);
	...
	for (i = 0; i < dyn_module->loaded_module->loaded_segments.size; i++) {
		...
		for (j = 0; j < fhdr->e_phnum; j++)
			if (dyn_module->phdr[j].p_vaddr
					== (Elf32_Addr) seg[i].phdr.p_vaddr) {
...
static void delete_DLIMP_Loaded_Module(DLIMP_Loaded_Module **pplm) {
	...
	DLIMP_Loaded_Segment *segments =
			(DLIMP_Loaded_Segment*) (loaded_module->loaded_segments.buf);
	...
	/*-----------------------------------------------------------------------*/
	if (loaded_module->name)
		DLIF_free(loaded_module->name);
	if (loaded_module->gsymtab)
		DLIF_free(loaded_module->gsymtab);
	loaded_module->gsymnum = 0;
	if (loaded_module->gstrtab)
		DLIF_free(loaded_module->gstrtab);
...
static DLIMP_Dynamic_Module *new_DLIMP_Dynamic_Module(LOADER_FILE_DESC *fd) {
	...
	/*-----------------------------------------------------------------------*/
	DLIMP_Dynamic_Module *dyn_module = (DLIMP_Dynamic_Module *) DLIF_malloc(
			sizeof(DLIMP_Dynamic_Module));
...
/*                                                                           */
/*****************************************************************************/
static DLIMP_Loaded_Module *detach_loaded_module(
		DLIMP_Dynamic_Module *dyn_module) {
...
static void delete_DLIMP_Dynamic_Module(DLIMP_Dynamic_Module **ppdm) {
	...
	dyn_module = *ppdm;
	if (dyn_module->name)
		DLIF_free(dyn_module->name);
	if (dyn_module->strtab)
		DLIF_free(dyn_module->strtab);
	if (dyn_module->symtab)
		DLIF_free(dyn_module->symtab);
	if (dyn_module->phdr)
		DLIF_free(dyn_module->phdr);
	if (dyn_module->dyntab)
		DLIF_free(dyn_module->dyntab);
...
static BOOL is_valid_elf_object_file(LOADER_FILE_DESC *fd,
	...
	/*------------------------------------------------------------------------*/
	if (!((dyn_module->strsz < fsz) && (dyn_module->symnum < fsz)
	...
	/*------------------------------------------------------------------------*/
	if (dyn_module->fhdr.e_type == ET_DYN && !dyn_module->symtab)
		return FALSE;
...
/*                                                                           */
/*****************************************************************************/
static inline BOOL dload_file_header(LOADER_FILE_DESC *fd,
		DLIMP_Dynamic_Module *dyn_module) {
	/*------------------------------------------------------------------------*/
...
#if 0
	printf("(dyn_module->fhdr).e_type is %d\n", (dyn_module->fhdr).e_type);
	printf("(dyn_module->fhdr).e_machine is %d\n", (dyn_module->fhdr).e_machine);
	printf("(dyn_module->fhdr).e_version is %d\n", (dyn_module->fhdr).e_version);
	printf("(dyn_module->fhdr).e_entry is %x\n", (dyn_module->fhdr).e_entry);
	printf("(dyn_module->fhdr).e_phoff is %d\n", (dyn_module->fhdr).e_phoff);
	printf("(dyn_module->fhdr).e_shoff is %d\n", (dyn_module->fhdr).e_shoff);
	printf("(dyn_module->fhdr).e_flags is 0x%x\n", (dyn_module->fhdr).e_flags);
	printf("(dyn_module->fhdr).e_ehsize is %d\n", (dyn_module->fhdr).e_ehsize);
	printf("(dyn_module->fhdr).e_phentsize is %d\n", (dyn_module->fhdr).e_phentsize);
	printf("(dyn_module->fhdr).e_phnum is %d\n", (dyn_module->fhdr).e_phnum);
	printf("(dyn_module->fhdr).e_shentsize is %d\n", (dyn_module->fhdr).e_shentsize);
	printf("(dyn_module->fhdr).e_shnum is %d\n", (dyn_module->fhdr).e_shnum);
	printf("(dyn_module->fhdr).e_shstrndx is %d\n", (dyn_module->fhdr).e_shstrndx);
#endif
	/*------------------------------------------------------------------------*/
	/* Determine target vs. host endian-ness.  Does header data need to be    */
	/* byte swapped?                                                          */
	/*------------------------------------------------------------------------*/
	dyn_module->wrong_endian = (dyn_module->fhdr.e_ident[EI_DATA]
			!= DLIMP_get_endian());
	...
	printf("ELF: %c%c%c\n", dyn_module->fhdr.e_ident[1],
			dyn_module->fhdr.e_ident[2], dyn_module->fhdr.e_ident[3]);
...
static void dload_program_header_table(LOADER_FILE_DESC *fd,
	...
	/*------------------------------------------------------------------------*/
	struct Elf32_Ehdr *fhdr = &(dyn_module->fhdr);
	dyn_module->phdr = (struct Elf32_Phdr*) (DLIF_malloc(
			fhdr->e_phnum * fhdr->e_phentsize));
...
static void copy_dynamic_table(LOADER_FILE_DESC *fd,
		DLIMP_Dynamic_Module *dyn_module, Elf32_Word dyn_seg_idx) {
	...
	num_elem = dyn_module->phdr[dyn_seg_idx].p_filesz
...
static BOOL process_dynamic_table(LOADER_FILE_DESC *fd,
	...
	if (symtab_offset) {
		...
		dyn_module->symtab = DLIF_malloc(
				dyn_module->symnum * sizeof(struct Elf32_Sym));
	...
	if (!soname_found) {
		...
		*dyn_module->name = '\0';
		...
		dyn_module->name = DLIF_malloc(
				strlen(dyn_module->strtab + soname_offset) + 1);
...
static BOOL check_circular_dependency(const char *dyn_mod_name) {
	...
	for (i = 0; i < DLIMP_module_dependency_list.size; i++)
		if (!strcmp(dyn_mod_name,
				((char**) (DLIMP_module_dependency_list.buf))[i])) {
			DLIF_error(DLET_MISC,
...
#if LOADER_DEBUG
	if (debugging_on)
		printf("Starting dload_static_executable() ...\n");
#endif
...
#if LOADER_DEBUG
	if (debugging_on)
		printf("Finished dload_static_executable()\n");
#endif
...
static void execute_module_pre_initialization(DLIMP_Dynamic_Module *dyn_module) {
	...
	if (dyn_module->preinit_arraysz != 0) {
		...
		TARGET_ADDRESS preinit_array_loc =
				(TARGET_ADDRESS) (dyn_module->dyntab[dyn_module->preinit_array_idx].d_un.d_ptr);

		/*---------------------------------------------------------------------*/
		/* Now make a loader-accessible copy of the .preinit_array section.    */
		/*---------------------------------------------------------------------*/
		int32_t i;
		int32_t num_preinit_fcns = dyn_module->preinit_arraysz
				/ sizeof(TARGET_ADDRESS);
		TARGET_ADDRESS *preinit_array_buf = (TARGET_ADDRESS *) DLIF_malloc(
...
static void execute_module_initialization(DLIMP_Dynamic_Module *dyn_module) {
	...
	if (dyn_module->init_idx != -1) {
		...
		/*---------------------------------------------------------------------*/
		TARGET_ADDRESS init_fcn =
				(TARGET_ADDRESS) (dyn_module->dyntab[dyn_module->init_idx].d_un.d_ptr);
	...
	if (dyn_module->init_arraysz != 0) {
		...
		TARGET_ADDRESS init_array_loc =
				(TARGET_ADDRESS) (dyn_module->dyntab[dyn_module->init_array_idx].d_un.d_ptr);

		/*---------------------------------------------------------------------*/
		/* Now make a loader-accessible copy of the .init_array section.       */
		/*---------------------------------------------------------------------*/
		int32_t i;
		int32_t num_init_fcns = dyn_module->init_arraysz
				/ sizeof(TARGET_ADDRESS);
		TARGET_ADDRESS *init_array_buf = (TARGET_ADDRESS *) DLIF_malloc(
				dyn_module->init_arraysz);
...
static int32_t relocate_dependency_graph_modules(LOADER_FILE_DESC *fd,
	...
	while (DLIMP_dependency_stack.size > 0) {
		DLIMP_Dynamic_Module *dyn_mod_ptr = dynamic_module_ptr_pop(
		...
		DLSYM_lookup_local_symtab("__c_args__", dyn_mod_ptr->symtab,
				dyn_mod_ptr->symnum, (Elf32_Addr *) &dyn_mod_ptr->c_args);
...
int32_t DLOAD_load(LOADER_FILE_DESC *fd, int argc, char** argv) {
	...
	DLSYM_lookup_local_symtab("__c_args__", dyn_module->symtab,
			dyn_module->symnum, (Elf32_Addr *) &dyn_module->c_args);
...
BOOL DLOAD_get_entry_names(uint32_t file_handle, int32_t *entry_pt_cnt,
	...
	/*------------------------------------------------------------------------*/
	loaded_module_ptr_Queue_Node* ptr;
	for (ptr = DLIMP_loaded_objects.front_ptr; ptr != NULL; ptr =
			ptr->next_ptr) {
...
/*                                                                           */
/*****************************************************************************/
static DLIMP_Loaded_Module *unlink_loaded_module(
...
static void execute_module_termination(DLIMP_Loaded_Module *loaded_module) {
	...
	if (loaded_module->fini_arraysz != 0) {
		...
		/*---------------------------------------------------------------------*/
		int32_t i;
		int32_t num_fini_fcns = loaded_module->fini_arraysz
				/ sizeof(TARGET_ADDRESS);
		TARGET_ADDRESS *fini_array_buf = (TARGET_ADDRESS *) DLIF_malloc(
...
static void remove_loaded_module(loaded_module_ptr_Queue_Node *lm_node) {
	...
	if (lm_node != DLIMP_loaded_objects.front_ptr)
		for (back_ptr = DLIMP_loaded_objects.front_ptr;
				back_ptr->next_ptr != lm_node; back_ptr = back_ptr->next_ptr)
			;
...
BOOL DLOAD_unload(uint32_t handle) {
	...
	for (lm_node = DLIMP_loaded_objects.front_ptr; lm_node != NULL; lm_node =
			lm_node->next_ptr) {
...
int32_t DLOAD_load_symbols(LOADER_FILE_DESC *fd) {
	...
	if (is_dsbt_module(dyn_module)) {
		dynamic_module_ptr_push(&DLIMP_dependency_stack, dyn_module);
		DLIF_register_dsbt_index_request(dyn_module->name,
				dyn_module->loaded_module->file_handle, dyn_module->dsbt_index);
		DLIF_assign_dsbt_indices();
		DLIF_update_all_dsbts();
...
uint32_t DLOAD_get_dsbt_size(int32_t file_handle) {
	...
	for (ptr = DLIMP_dependency_stack.top_ptr; ptr != NULL; ptr =
			ptr->next_ptr) {
	...
	for (ptr = DLIMP_dependency_stack.top_ptr; ptr != NULL; ptr =
		...
		if (dmp->loaded_module->file_handle == file_handle) {
			BOOL stat = DLSYM_lookup_local_symtab("__TI_STATIC_BASE",
					dmp->symtab, dmp->symnum, (Elf32_Addr *) static_base);
			return stat;
		}
	}
