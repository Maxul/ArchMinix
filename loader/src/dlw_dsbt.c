...
BOOL DLIF_register_dsbt_index_request(const char *requestor_name,
		int32_t requestor_file_handle, int32_t requested_dsbt_index) {
	...
	if (requested_dsbt_index != DSBT_INDEX_INVALID) {
		...
		for (ptr = DSBT_index_request_queue.front_ptr; ptr != NULL;
				ptr = ptr->next_ptr) {
			...
			if (requested_dsbt_index == existing_request->requested_index) {
				DLIF_error(DLET_MISC,
						...
						requested_dsbt_index, requestor_name,
	...
	/*------------------------------------------------------------------------*/
	new_request = (DSBT_Index_Request *) DLIF_malloc(
			sizeof(DSBT_Index_Request));
	...
	if (!DLOAD_get_dsbt_base(requestor_file_handle, &new_request->dsbt_base)) {
		DLIF_error(DLET_MISC, "Could not resolve DSBT base value for %s",
				requestor_name);
		return FALSE;
	}

	if (!DLOAD_get_static_base(requestor_file_handle,
			&new_request->static_base)) {
		DLIF_error(DLET_MISC, "Could not resolve static base value for %s",
				requestor_name);
		return FALSE;
	}
...
void DLIF_assign_dsbt_indices(void) {
	...
	for (; ptr != NULL; ptr = next_ptr) {
		...
		if (curr_req->requested_index == DSBT_INDEX_INVALID)
			continue;
